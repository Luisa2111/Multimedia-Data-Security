...
if __name__ == "__main__":
    ...
    for im in range(101):
        ...
        embeddingSS.embedding(name_image, mark, name_output=name_out, alpha=0.5)
        name_out = path_out + 'wat_' + img + '-06' + '.bmp'
        embeddingSS.embedding(name_image, mark, name_output=name_out, alpha= 1)
        name_out = path_out + 'wat_' + img + '-07' + '.bmp'
        embedding.embedding(name_image, mark, name_output=name_out, alpha= 0.2)
        name_out = path_out + 'wat_' + img + '-08' + '.bmp'
        embedding.embedding(name_image, mark, name_output=name_out, alpha=0.7)
        name_out = path_out + 'wat_' + img + '-09' + '.bmp'
        embedding.embedding(name_image, mark, name_output=name_out, alpha=1.4)
